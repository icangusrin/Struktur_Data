...
transaksi_file = 'transaksi_baru.csv'  # memakai file hasil perbaikan
...
def tambah_transaksi(id_produk, jumlah, tipe):
    ...
    file_empty = not os.path.exists(transaksi_file) or os.stat(transaksi_file).st_size == 0
        ...
        with open(transaksi_file, mode='r', newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    ...
                    print(f"Kesalahan parsing transaksi: {row} -> {e}")
    return tree
...
def cetak_transaksi(row):
    print(f"{row['tanggal']} {row['waktu']} | {row['tipe']} | ID Produk: {row['id_produk']} | Jumlah: {row['jumlah']}")
